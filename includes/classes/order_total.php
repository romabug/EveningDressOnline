        ...
        if ($return_html == true) {
          for ($i=0; $i<$size; $i++) {
            ...
            $output_string .= '<tr>'."\n".
                              ...
                              '<td id="'.str_replace('_', '-', $GLOBALS[$class]->code).'">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                              '</tr>';
            }else{
            $output_string .= '<tr>'."\n".
                              '<td class="pad_b2" colspan="2">'. "\n" .
                              '<div class="g_t_c green big b border_t" style="margin-left: 290px; display: none;">'.
                              'Total Saving: <span id="Total_Saving">US$ 0.00</span></div>'.
                              '<h3 style="margin-left: 290px;" class="g_t_c">' . $GLOBALS[$class]->output[$i]['title'].'<span id="'.str_replace('_', '-', $GLOBALS[$class]->code).'">' . $GLOBALS[$class]->output[$i]['text'] . '</span></h3></td>' . "\n" .
                              '</tr>';          	
            }
          }
        } else {
