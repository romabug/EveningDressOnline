...
class currencies extends base {
  ...
  function display_price($products_price, $products_tax, $quantity = 1) {
	...
	function display_symbol_left($code){
		return $this->currencies[$code]['symbol_left'];
	}
	
  function noSymbolFormat($number, $calculate_currency_value = true, $currency_type = '', $currency_value = '') {

    if (empty($currency_type)) $currency_type = $_SESSION['currency'];

    if ($calculate_currency_value == true) {
      $rate = (zen_not_null($currency_value)) ? $currency_value : $this->currencies[$currency_type]['value'];
      $format_string = number_format(zen_round($number * $rate, $this->currencies[$currency_type]['decimal_places']), $this->currencies[$currency_type]['decimal_places'], $this->currencies[$currency_type]['decimal_point'],'');
    } else {
      $format_string = number_format(zen_round($number, $this->currencies[$currency_type]['decimal_places']), $this->currencies[$currency_type]['decimal_places'], $this->currencies[$currency_type]['decimal_point'],'');
    }
  ...
  function noSymbolDisplayPrice($products_price, $products_tax, $quantity = 1){
