...
$qty = $this->adjust_quantity($qty, $products_id, 'shopping_cart');
  ...
  function calculate() {
    ...
    while (list($products_id, ) = each($this->contents)) {
      ...
      // products price
      $product_query = "select products_id, products_price, products_price_sample, products_tax_class_id, products_weight,
      ...
      if ($product = $db->Execute($product_query)) {
        ...
        $products_price = ($product->fields['products_price'] == 0 ? $product->fields['products_price_sample'] : $product->fields['products_price'] );
