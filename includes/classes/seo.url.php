      ...
      if ($use_dir_ws_catalog) {
		...
		function stock_href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true, $static = false, $use_dir_ws_catalog = true) {
			...
			if (!$this->not_null($page)) {
				die('</td></tr></table></td></tr></table><br /><br /><strong class="note">Error!<br /><br />No es posible determinar el enlace de la p醙ina!</strong><br /><br />');
			}
			...
			if ($connection == 'NONSSL') {
				...
				die('</td></tr></table></td></tr></table><br /><br /><strong class="note">Error!<br /><br />No es posible determinar el m閠odo de conexi髇 en un v韓culo!<br /><br />M閠odos conocidos: NONSSL SSL</strong><br /><br />');
			}
...
//						case ($page == FILENAME_PRODUCT_INFO && !$this->is_attribute_string($params)):
						case ($page == FILENAME_PRODUCT_INFO):
							...
							break;
						case ($page == FILENAME_PRODUCT_FREE_SHIPPING_INFO):
							$url = $this->make_url($page,$this->get_prodcut_name($p2[1]),$p2[0],$p2[1],'.html',$separator);
	...
	function get_product_name($pID){
		switch(true){
     	case ($this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true' && defined('PRODUCT_NAME_' . $pID)):
			...
			default:
				$sql = "SELECT metatags_title as pName 
 ...
 */	
	function get_category_name(&$cID){
		...
		$cID = $full_cPath;
		return  SEO_COMMON_KEYWORDS .'-'.$return;
	} # end function
 ...
 * @version 1.0
 */	
 // 修改 翻页以后，产品URL 还是显示原来产品名的问题，是缓存的问题。
 // 修改过的地方：原来是： 			$sql = "SELECT p.products_id as id, pd.products_name as name 
