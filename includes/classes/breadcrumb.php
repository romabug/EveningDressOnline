      ...
      if ((isset($this->_trail[$i]['link']) && zen_not_null($this->_trail[$i]['link']) && !$skip_link )) {
        ...
        if ($this->_trail[$i]['title'] == HEADER_TITLE_CATALOG) {
          $trail_string .= '  <a href="' . HTTP_SERVER . DIR_WS_CATALOG . '">' . $this->_trail[$i]['title'] . '</a>';
        	...
        	if ($_GET['main_page'] == 'index' || isset($_GET['products_id'])){
          $trail_string .= '  <a href="' . $this->_trail[$i]['link'] . '">'. SEO_COMMON_KEYWORDS .' '.$this->_trail[$i]['title'] . '</a>';
        	}else{
        ...
        $trail_string .= '<a href="' . $this->_trail[$i]['link'] . '"><span class="red">'. SEO_COMMON_KEYWORDS .' '.$this->_trail[$i]['title'].'</span></a>';
				}else{
        $trail_string .= '<span class="red">'. $this->_trail[$i]['title'].'</span>';
				}
      }
