...
// add attributes price to price
  ...
  function zen_get_products_retail_price($products_id) {
    global $db;
      $product_check = $db->Execute("select products_price_retail, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");

// is there a products_price to add to attributes
      $products_price = $product_check->fields['products_price_retail'];
...
function zen_get_products_sample_price($products_id) {
    global $db;
      $product_check = $db->Execute("select products_price_sample, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");

// is there a products_price to add to attributes
      $products_price = $product_check->fields['products_price_sample'];
...
// show case only
    ...
    if ($display_special_price) {
      $show_normal_price = '<del>' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </del>';
      if ($display_sale_price && $display_sale_price != $display_special_price) {
        $show_special_price = '&nbsp;' . '<strong class="red">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</strong>';
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          $show_special_price = '&nbsp;' . '<strong class="red">' . '<s>' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . '</strong>';
        } else {
          $show_special_price = '&nbsp;' . '<strong class="red">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</strong>';
        }
        $show_sale_price = '';
      ...
      if ($display_sale_price) {
        $show_normal_price = '<span class="del">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </del>';
        $show_special_price = '';
        $show_sale_price = '<br />' . '<span class="productSalePrice">' . PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
      } else {
        if ($product_check->fields['product_is_free'] == '1') {
    ...
    if ($display_normal_price == 0) {
      // don't show the $0.00
			// Delete $show_sale_discount 
      $final_display_price = '<br/>'.$show_special_price . '<br/>'.$show_sale_price ;
    } else {
      $final_display_price = '<br/>'.$show_normal_price . '<br/>'.$show_special_price . $show_sale_price ;
    }
...
// Display Price Retail
// Specials and Tax Included
  function zen_get_products_display_final_price($products_id) {
    global $db, $currencies;
...
// show case only
    ...
    if ($display_normal_price == 0) {
      // don't show the $0.00
			// Delete $show_sale_discount 
      $final_display_price = $show_special_price .$show_sale_price ;
    } else {
      $final_display_price = $show_normal_price .$show_special_price . $show_sale_price ;
    }
