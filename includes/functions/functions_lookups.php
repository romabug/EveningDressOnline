  ...
  function zen_get_countries($countries_id = '', $with_iso_codes = false) {
    ...
    if (zen_not_null($countries_id)) {
      ...
      while (!$countries_values->EOF) {
          ...
          array_unshift(&$countries_array,array('countries_id' => $countries_values->fields['countries_id'],
 ...
 */
  ...
  function zen_get_products_weight($product_id) {
    global $db;
    $product = $db->Execute("select products_weight
                             from " . TABLE_PRODUCTS . "
                             where products_id = '" . (int)$product_id . "'");

    return $product->fields['products_weight'];
  }
/**
...
/*
 * Find basis_category name from ID, in indicated language
 */
  function zen_get_basis_category_name($category_id, $fn_language_id) {
    global $db;
    ...
    $category = $db->Execute($category_query);
    	...
    	$category_name =  $category->fields['categories_name'];
		}else{
    	$category_name = zen_get_basis_category_name($category->fields['parent_id'], $fn_language_id);
		}
...
/*
 * Find basis_category_id name from ID, in indicated language
 */
 ...
 */
  ...
  function zen_get_category_bottom_description($category_id, $language_id) {
    global $db;
    $category = $db->Execute("select categories_bottom_description
                              from " . TABLE_CATEGORIES_DESCRIPTION . "
                              where categories_id = '" . (int)$category_id . "'
                              and language_id = '" . (int)$language_id . "'");

    return $category->fields['categories_bottom_description'];
  }
...
//    $new_range = ' and date_format(p.products_date_available, \'%Y%m%d\') >' . $zc_new_date;
  ...
  function zen_get_order_no($orders_id){
  	global $db;
  	$query = "SELECT o.`order_no` FROM orders o
WHERE o.`orders_id` =".$orders_id;
  ...
  function zen_get_categories_displayTypes($categories_id){
  	global $db;
  	$sql = "SELECT c.`categories_display_type` FROM categories c
WHERE c.`categories_id` = ".$categories_id;
  	$sqlArray = $db->Execute($sql);
  	return $sqlArray->fields['categories_display_type'];
  }
  
function zen_get_categories_listTypes($categories_id){
    global $db;
    $sql = "SELECT c.`categories_list_types` FROM categories c
WHERE c.`categories_id` = ".$categories_id;
    $sqlArray = $db->Execute($sql);
    return $sqlArray->fields['categories_list_types'];
  }
...
/*
* Find whether the product Have Free Shipping Types
* Add Showq
*
*/
   function zen_get_freeshipping_same_products($products_name){
   	global $db,$template_dir;
   	$string = '';
   	$query = "SELECT p.`products_id`,pd.`products_name` FROM ". TABLE_PRODUCTS." p , ". TABLE_PRODUCTS_DESCRIPTION." pd WHERE pd.`products_id`=p.`products_id` AND pd.`products_name` LIKE '" . $products_name ."%free shipping' ";
   	$products = $db->Execute($query);
    if ($products->RecordCount() > 0){
    	$string .= '<li class="big b black"><a href="'.zen_href_link(zen_get_info_page($products->fields['products_id']),'products_id='.$products->fields['products_id']).'">Look for free shipping click here</a> &gt; <a href="'.zen_href_link(zen_get_info_page($products->fields['products_id']),'products_id='.$products->fields['products_id']).'"><img border="0" class="g_t_m" src="includes/templates/'.$template_dir.'/images/button/bulksale.gif"/></a></li>';
    }else {
    	$string = '';
    }
    return $string;
