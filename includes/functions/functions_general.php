...
// add default logical operators if needed
    ...
    $operator_count = 0;
    $balance = 0;
    $objectsNumTwo = count($objects);
    for($i=0; $i<$objectsNumTwo; $i++) {
 ...
 */
  function zen_js_zone_list($country, $form, $field) {
    ...
    while (!$countries->EOF) {
      ...
      $num_country++;
      $countries->MoveNext();
      $output_string .= '    show("zone_id_tr");hide("zone_id_tr1");theForm.state.value="";' . "\n" ;
    }
    $output_string .= '  } else {' . "\n" .
                      '    hide("zone_id_tr");show("zone_id_tr1");' . "\n" .
                      '  }' . "\n";
 ...
 */
 	...
 	function zen_clipped_string($string,$needle=' ',$strlen='24'){
		$final_string ='';
		$temp_array = split($needle, $string);
...
// This function is not available in all PHP versions so we test it before using it.
  function zen_class_exists($class_name) {
	...
	function filterName($array){
		...
		$n = count($array);
		for($i=0;$i<$n;$i++){
			if (key($array) != 'firstname' and key($array) != 'lastname'){
				 $newArray[key($array)]=$array[key($array)];
			}
			next($array);
			//print_r(key($array));
		}
