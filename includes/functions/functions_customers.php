...
// Text Mode
    ...
    $statecomma = '';
    $streets = $street;
    if ($suburb != '') $streets = $street . ',' . $suburb;
    if ($country == '') {
      if (is_array($address['country'])) {
        $country = zen_output_string_protected($address['country']['countries_name']);
      } else {
...
// TABLES: customers, address_book
  ...
  function zen_get_address_fields($customers_id, $address_id = 1){
  	global $db;
    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,
                             entry_company as company, entry_street_address as street_address,
                             entry_suburb as suburb, entry_city as city, entry_postcode as postcode,entry_phone as phone,
...
// validate customer matches session
  ...
  function zen_get_customer_name($customer_id){
    ...
    $last_name = $customer->fields['customers_lastname'] ;
    return $last_name;
  }
  
  function zen_get_customer_email($customer_id){
    global $db;
    $customer_query = "select customers_email_address
                         from " . TABLE_CUSTOMERS . "
                         where customers_id = '" . $customer_id . "'";
