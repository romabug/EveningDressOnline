...
// This is done after quote-calcs in order to include Tare info accurately.  NOTE: tare values are *not* included in weights shown on-screen.
  ...
  switch (true) {
      case (SHOW_TOTALS_IN_CART == '1'):
      $totalsDisplay = TEXT_TOTAL_ITEMS . '<b>'.$_SESSION['cart']->count_contents() .'</b>'. TEXT_TOTAL_WEIGHT .'<b>'. $shipping_estimator_display_weight .'</b>'. TEXT_PRODUCT_WEIGHT_UNIT . TEXT_TOTAL_AMOUNT .'<b>'. $currencies->format($_SESSION['cart']->show_total()).'</b>';
      break;
      case (SHOW_TOTALS_IN_CART == '2'):
      $totalsDisplay = TEXT_TOTAL_ITEMS . '<b>'. $_SESSION['cart']->count_contents() .'</b>'. ($shipping_estimator_display_weight > 0 ? TEXT_TOTAL_WEIGHT .'<b>'.  $shipping_estimator_display_weight .'</b>'. TEXT_PRODUCT_WEIGHT_UNIT : '') . TEXT_TOTAL_AMOUNT .'<b>'. $currencies->format($_SESSION['cart']->show_total()).'</b>';
      break;
      case (SHOW_TOTALS_IN_CART == '3'):
      $totalsDisplay = TEXT_TOTAL_ITEMS . '<b>'. $_SESSION['cart']->count_contents() .'</b>'. TEXT_TOTAL_AMOUNT .'<b>'. $currencies->format($_SESSION['cart']->show_total()).'</b>';
      break;
  }
