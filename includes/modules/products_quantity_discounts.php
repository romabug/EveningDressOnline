...
// retrieve the list of discount levels for this product
  ...
  if ($products_discounts_query->RecordCount() <=0 ) {
  	$products_discounts_query = $db->Execute("SELECT * FROM " . TABLE_CATEGORIES_DISCOUNT_QUANTITY ." WHERE categories_id = " . (int)$current_category_id . " AND discount_qty !=0" . " ORDER BY discount_qty" );
  }
...
while (!$products_discounts_query->EOF) {
  ...
  switch ($products_discount_type) {
    ...
    case '1':
      ...
      $quantityDiscounts[$columnCount]['discount'] = ($products_discounts_query->fields['discount_price']/100)*100 . '%';
    break;
    // actual price
    case '2':
      if ($products_discount_type_from == '0') {
      ...
      $quantityDiscounts[$columnCount]['discount'] = (floor($quantityDiscounts[$columnCount]['discounted_price']/$display_price))*100 . '%';
    break;
    // amount offprice
    case '3':
      if ($products_discount_type_from == '0') {
