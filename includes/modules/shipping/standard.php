...
// class constructor
    function standard() {
      $this->code = 'standard';
      $this->title = MODULE_SHIPPING_STANDARD_TEXT_TITLE;
      $this->description = MODULE_SHIPPING_STANDARD_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_SHIPPING_STANDARD_SORT_ORDER;
      $this->icon = '';
      $this->tax_class = MODULE_SHIPPING_STANDARD_TAX_CLASS;
      $this->tax_basis = MODULE_SHIPPING_STANDARD_TAX_BASIS;

      // disable only when entire cart is free shipping
      if (zen_get_shipping_enabled($this->code)) {
        $this->enabled = ((MODULE_SHIPPING_STANDARD_STATUS == 'True') ? true : false);
      }
...
// class methods
    ...
    $order_total_amount = $_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices() ;
      ...
      if ($dest_zone == 0) {
        ...
        $shipping = -1;
        $zones_cost = constant('MODULE_SHIPPING_STANDARD_COST_' . $dest_zone);

        $zones_table = split("[:,]" , $zones_cost);
        $size = sizeof($zones_table);
        $done = false;
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_STANDARD_METHOD) {
        	  case (MODULE_SHIPPING_STANDARD_METHOD == 'Weight'):
              if (round($shipping_weight,9) <= $zones_table[$i]) {
                ...
                case (1):
                  ...
                  break;
                case (2):
                  $show_box_weight = ' (' . number_format($shipping_weight * $shipping_num_boxes,2) . MODULE_SHIPPING_STANDARD_TEXT_UNITS . ')';
                  break;
                default:
                  $show_box_weight = ' (' . $shipping_num_boxes . ' x ' . number_format($shipping_weight,2) . MODULE_SHIPPING_STANDARD_TEXT_UNITS . ')';
                  break;
                }

                //$shipping_method = MODULE_SHIPPING_STANDARD_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_STANDARD_TEXT_UNITS : '');
                $shipping_method = MODULE_SHIPPING_STANDARD_TEXT_WAY . ' ' . $dest_country . $show_box_weight;
        ...
        if (strstr($zones_table[$i+1], '%')) {
          ...
          $shipping = $zones_table[$i+1];
      	    ...
      	    break;
        	  case (MODULE_SHIPPING_STANDARD_METHOD == 'Price'):
// shipping adjustment
              if (($_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices()) <= $zones_table[$i]) {
                $shipping = $zones_table[$i+1];
                $shipping_method = MODULE_SHIPPING_STANDARD_TEXT_WAY . ' ' . $dest_country;
        if (strstr($zones_table[$i+1], '%')) {
          $shipping = ($zones_table[$i+1]/100) * $order_total_amount;
        } else {
          $shipping = $zones_table[$i+1];
	          ...
	          break;
        	  case (MODULE_SHIPPING_STANDARD_METHOD == 'Item'):
// shipping adjustment
        ...
        if ($shipping == -1) {
          $shipping_cost = 0;
          $shipping_method = MODULE_SHIPPING_STANDARD_UNDEFINED_RATE;
        } else {
          switch (MODULE_SHIPPING_STANDARD_METHOD) {
        	  case (MODULE_SHIPPING_STANDARD_METHOD == 'Weight'):
              // charge per box when done by Price
              $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_STANDARD_HANDLING_' . $dest_zone);
              break;
        	  case (MODULE_SHIPPING_STANDARD_METHOD == 'Price'):
              // don't charge per box when done by Price
              $shipping_cost = ($shipping) + constant('MODULE_SHIPPING_STANDARD_HANDLING_' . $dest_zone);
            break;
        	  case (MODULE_SHIPPING_STANDARD_METHOD == 'Item'):
      ...
      $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_STANDARD_TEXT_TITLE,
                            'description' => MODULE_SHIPPING_STANDARD_TEXT_DESCRIPTION,
      ...
      if (strstr(MODULE_SHIPPING_STANDARD_SKIPPED, $dest_country)) {
        // don't show anything for this country
        $this->quotes = array();
      } else {
        if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_STANDARD_INVALID_ZONE;
      }
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_STANDARD_STATUS'");
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_STANDARD_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Calculation Method', 'MODULE_SHIPPING_STANDARD_METHOD', 'Weight', 'Calculate cost based on Weight, Price or Item?', '6', '0', 'zen_cfg_select_option(array(\'Weight\', \'Price\', \'Item\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_STANDARD_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '0', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Tax Basis', 'MODULE_SHIPPING_STANDARD_TAX_BASIS', 'Shipping', 'On what basis is Shipping Tax calculated. Options are<br />Shipping - Based on customers Shipping Address<br />Billing Based on customers Billing address<br />Store - Based on Store address if Billing/Shipping Zone equals Store zone', '6', '0', 'zen_cfg_select_option(array(\'Shipping\', \'Billing\', \'Store\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_STANDARD_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Skip Countries, use a comma separated list of the two character ISO country codes', 'MODULE_SHIPPING_STANDARD_SKIPPED', '', 'Disable for the following Countries:', '6', '0', 'zen_cfg_textarea(', now())");
      ...
      for ($i = 1; $i <= $this->num_zones; $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_STANDARD_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".<br />Set as 00 to indicate all two character ISO country codes that are not specifically defined.', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_STANDARD_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights/prices. Example: 3:8.50,7:10.50,... Weight/Price less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.<br />You can end the last amount as 10000:7% to charge 7% of the Order Total', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_STANDARD_HANDLING_" . $i."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
      }
    }
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_STANDARD_STATUS', 'MODULE_SHIPPING_STANDARD_METHOD', 'MODULE_SHIPPING_STANDARD_TAX_CLASS', 'MODULE_SHIPPING_STANDARD_TAX_BASIS', 'MODULE_SHIPPING_STANDARD_SORT_ORDER', 'MODULE_SHIPPING_STANDARD_SKIPPED');

      for ($i=1; $i<=$this->num_zones; $i++) {
        $keys[] = 'MODULE_SHIPPING_STANDARD_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_STANDARD_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_STANDARD_HANDLING_' . $i;
      }
