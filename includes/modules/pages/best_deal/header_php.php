...
//  $display_limit = zen_get_beat_deal_timelimit();
  ...
  $best_deal_split = new splitPageResults($best_deal_query_raw, (isset($_GET['pagesize']) ? $_GET['pagesize'] : 24));
...
//check to see if we are in normal mode ... not showcase, not maintenance, etc
  ...
  if($best_deal->RecordCount()>0){
   ...
   $best_deal_query_raw2 = "SELECT p.products_id, p.products_type, pd.products_name, p.products_image, p.products_price,
                                    p.products_tax_class_id, p.products_date_added, m.manufacturers_name, p.products_model,
                                    p.products_quantity, p.products_weight, p.product_is_call,
                                    p.product_is_always_free_shipping, p.products_qty_box_status,
                                    p.master_categories_id,p.product_is_wholesale,p.product_wholesale_min,p.products_price_sample,
  ...
  $best_deal_query_raw2 = $db->bindVars($best_deal_query_raw2, ':languageID', $_SESSION['languages_id'], 'integer');
  $best_deal_split2 = new splitPageResults($best_deal_query_raw2, (isset($_GET['pagesize']) ? $_GET['pagesize'] : 24));
...
//check to see if we are in normal mode ... not showcase, not maintenance, etc
  ...
  $best_deal2 = $db->Execute($best_deal_split2->sql_query);
  if($best_deal2->RecordCount()>0){
    $row = 0;
    while (!$best_deal2->EOF) {
      if ($best_deal2->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
        $list_box_contents2[$row]['products_image'] = '';
      } else {
        $list_box_contents2[$row]['products_image'] = $best_deal2->fields['products_image'] ;
      }
      $list_box_contents2[$row]['products_name'] = $best_deal2->fields['products_name'];
      $list_box_contents2[$row]['products_description'] = zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($best_deal2->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION);
      $list_box_contents2[$row]['products_price'] = zen_get_products_base_price($best_deal2->fields['products_id']);
      $list_box_contents2[$row]['actual_price'] = $currencies->display_price(zen_get_products_actual_price($best_deal2->fields['products_id']),zen_get_tax_rate($product_check->fields['products_tax_class_id']));
      $list_box_contents2[$row]['products_status']=$best_deal2->fields['products_status'];
      if ($best_deal2->fields['product_is_always_free_shipping'] == 0) {
      ...
      $list_box_contents2[$row]['products_quantity_order_min'] = $best_deal2->fields['products_quantity_order_min'];
      $list_box_contents2[$row]['products_id'] = $best_deal2->fields['products_id'];
      $list_box_contents2[$row]['products_quantity'] = $best_deal2->fields['products_quantity'];
      
      $list_box_contents2[$row]['products_price_retail'] = $currencies->display_price($best_deal2->fields['products_price_retail'],zen_get_tax_rate($product_check->fields['products_tax_class_id']));
      $list_box_contents2[$row]['products_price_sample'] = $currencies->display_price($best_deal2->fields['products_price_sample'],zen_get_tax_rate($product_check->fields['products_tax_class_id']));
      $list_box_contents2[$row]['product_is_wholesale'] = $best_deal2->fields['product_is_wholesale'];
      $list_box_contents2[$row]['product_wholesale_min'] = $best_deal2->fields['product_wholesale_min'];
      $best_deal2->MoveNext();
      $row++;
    }
  }
