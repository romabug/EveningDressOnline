...
if (isset($_GET['action']) && ($_GET['action'] == 'send')) {
  ...
  if ($zc_validate_email and !empty($enquiry) and !empty($name)) {
    ...
    zen_redirect(zen_href_link(FILENAME_POPUP_ASK_A_QUESTION2, 'action=success&products_id='.$_GET['products_id']));
  } else {
    $error = true;
    if ($error) {
...
$define_page = zen_get_file_directory(DIR_WS_LANGUAGES . $_SESSION['language'] . '/html_includes/', FILENAME_DEFINE_ASK_A_QUESTION2, 'false');
