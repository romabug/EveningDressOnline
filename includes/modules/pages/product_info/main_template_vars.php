 ...
 */
  ...
  if ( $res->fields['total'] < 1 ) {
	...
	$themes_sql = "select c.categories_banner_2_link from " . TABLE_CATEGORIES . " c where c.categories_id = :categoriesID";
	$themes_sql = $db->bindVars($themes_sql, ':categoriesID', $current_category_id, 'integer');
    $themes = $db->Execute($themes_sql);
	$themes_mark = "";
	$themes_mark = $themes->fields['categories_banner_2_link'];
	if($themes_mark!=""){ $tpl_page_body = '/tpl_product_info_display_'. $themes_mark .'.php'; }
    ...
    $sql = "select p.products_id, pd.products_name,
                  ...
                  p.products_qty_box_status,p.products_quantity_order_min,
	...
	// 加一个变量调用TITLT， 调用关键字
   // $meta_tags_add_neirong = $product_info->fields['metatags_description'];
...
// if review must be approved or disabled do not show review
    ...
    $reviews_query = "select r.reviews_id,r.customers_name,r.reviews_rating,r.date_added,r.reviews_is_featured,rd.reviews_title,rd.reviews_text from " . TABLE_REVIEWS . " r, "
  ...
  $categories_discount = $db->Execute("SELECT * FROM ".TABLE_CATEGORIES." WHERE categories_id = ".$current_category_id);

  if ($products_discount_type == 0){
	  $products_discount_type = $categories_discount->fields['categories_discount_type'];
	  $products_discount_type_from = $categories_discount->fields['categories_discount_type_from'];
  }else{
