...
'text' => '<div class="listingProductImage"><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $xsell_query->fields['products_image'], $xsell_query->fields['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div>'. 
		  '<div class="itemTitle"><h3 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . $xsell_query->fields['products_name'] . '</a></h3></div>'. (XSELL_DISPLAY_PRICE=='true'? '<div class="price">'.zen_get_products_display_price($xsell_query->fields['products_id']).'</div>':'') );
  $col ++;
